...
def setup(app):
    app.add_config_value("detail_view_config", {}, "html")
...
def _save_js_file(app, target):
    config = app.config.detail_view_config

    final_config = {
        "selector": config.get("selector", "div.needs_head > div.line"),
        "not_selector": config.get("not_selector", "h1 a, h2 a, h3 a, h4 a, h5 a"),
    ...
    TEMPLATE_FILE = "template_sphinx_detail_view.js"
    try:
...
def install_lib_static_files(app, env):
    ...
    extra_files = ["assets/sphinx_detail_view.css"]
    register_files = [
        "assets/template_sphinx_detail_view.js",
        "assets/sphinx_detail_view.css",
    ]
    ...
    dest_path = os.path.join(static_path, "assets/template_sphinx_detail_view.js")
